...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
...

    {
        ...
        if (recipients[0].useSwiftTX && total > sporkManager.GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 FAW.").arg(sporkManager.GetSporkValue(SPORK_5_MAX_VALUE)),
        ...
        QSettings settings;
        bool autoFee = settings.value("autoFee", false).toBool();

        if (!fCreated && autoFee) {
            vecSend.back().second -= nFeeRequired + 100;
            fCreated = wallet->CreateTransaction(vecSend, *newTx, *keyChange, nFeeRequired, strFailReason, coinControl, recipients[0].inputType, recipients[0].useSwiftTX, nFeeRequired + 100);
        }
