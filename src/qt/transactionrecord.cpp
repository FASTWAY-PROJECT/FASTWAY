...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->zpivTracker->HasMintTx(hash))) {
        ...
        } else if (mine = wallet->IsMine(wtx.vout[1])) {
            // PIV stake reward
            ...
            sub.credit = nNet;
            // detect MN reward
            if (nIndexMN > 1 && ExtractDestination(wtx.vout[nIndexMN].scriptPubKey, destMN) && IsMine(*wallet, destMN) && wtx.vout[nIndexMN].nValue < nNet) {
                // add Stake
                sub.credit -= wtx.vout[nIndexMN].nValue;
                parts.append(sub);
                // add MN
                mine = wallet->IsMine(wtx.vout[nIndexMN]);
                sub.involvesWatchAddress = mine & ISMINE_WATCH_ONLY;
