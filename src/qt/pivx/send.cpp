    ...
    btnContacts(new QPushButton())
{
    ...
    /* Title */
    ui->labelTitle->setText(tr("Send coins"));
    setCssProperty(ui->labelTitle, "text-title-screen");
    ui->labelTitle->setFont(fontLight);

    /* Button Group */
    ui->pushLeft->setText("FAW");
    setCssProperty(ui->pushLeft, "btn-check-left");
    ui->pushLeft->setChecked(true);
    ui->pushRight->setText("zFAW");
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (FAW) or private coins (zFAW)"));
    ...
    /* Address */
    ui->labelSubtitleAddress->setText(tr("Enter a FastWay address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 FAW");
    ...
    setCssProperty(ui->tbAmountRemaining, "text-body1");
    connect(ui->tbAmountRemaining, SIGNAL(clicked()), this, SLOT(onAmountClicked()));
...
void SendWidget::refreshAmounts() {
    ...
    if (CoinControlDialog::coinControl->HasSelected()){
        ...
        // Wallet's balance
        totalAmount = (isZpiv ? walletModel->getZerocoinBalance() : walletModel->getBalance() - walletModel->getLockedBalance()) - total;
...
void SendWidget::onAmountClicked() {
    QMutableListIterator<SendMultiRow*> it(entries);
    it.toBack();
    SendMultiRow* entry = it.previous();
    CAmount amount = entry->getAmountValue();
    entry->setAmount(BitcoinUnits::format(nDisplayUnit, amount + totalAmount, false, BitcoinUnits::separatorNever, true));
}
...
bool SendWidget::sendZpiv(QList<SendCoinsRecipient> recipients){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        emit message(tr("Spend Zerocoin"), tr("zFAW is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    // use mints from zFAW selector if applicable
    ...
    if (walletModel->sendZpiv(
        ...
        inform(tr("zFAW transaction sent!"));
        ...
        if (receipt.GetStatus() == ZPIV_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zFAW require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zFAW transaction
        ...
        emit message("zFAW transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isPIV){
        ...
        if (walletModel->getZerocoinBalance() > 0) {
            ...
            inform(tr("You don't have any zFAW in your balance to select."));
        }
    }
}
