    ...
    walletModel(model)
{
    ...
    ui->lineEditPort->setPlaceholderText("e.g 55125");
    ...
    if(walletModel->isTestnet()){
        ...
        ui->lineEditPort->setText("55126");
    } else {
        ui->lineEditPort->setText("55125");
    }
...
bool MasterNodeWizardDialog::createMN(){
    if (walletModel) {
        ...
        // Validate IP address
        CNetAddr nodeAddress = CNetAddr(addressStr.toStdString());
        if (!nodeAddress.IsValid()) {
            returnStr = tr("Invalid IP address");
            return false;
        }
        if (nodeAddress.IsIPv6()) {
            addressStr = QString::fromStdString(nodeAddress.ToString()).insert(0,"[").append("]");
        }
        ...
        SendCoinsRecipient sendCoinsRecipient(QString::fromStdString(address.ToString()), addressLabel, CAmount(25000) * COIN, "");
            ...
            if (boost::filesystem::exists(pathBootstrap)) {
                ...
                if (lineCopy.size() == 0) {
                    lineCopy = "# Masternode config file\n"
                               ...
                               "# Example: mn1 127.0.0.2:55125 93HaYBVUCYjEMeeH1Y4sBGLALQZE1Yc1K64xiqgX37tGBDQL8Xg 2bcd3c84c84f87eaa86e4e56834c92927a07f9e18718810b92e0d0324456a67c 0"
                               "#";
                }
                lineCopy += "\n";
                ...
                uint256 mnTxHash;
                mnTxHash.SetHex(txID);
                COutPoint outpoint = COutPoint(mnTxHash, indexOut);
