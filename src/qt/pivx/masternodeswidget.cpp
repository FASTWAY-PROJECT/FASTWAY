...
public:
    ...
    void init(QWidget* holder,const QModelIndex &index, bool isHovered, bool isSelected) const override{
        ...
        QString ip_address = index.sibling(index.row(), MNModel::IP_ADDRESS).data(Qt::DisplayRole).toString();
        ...
        row->updateView("IP Address: " + ip_address, label, " - " + address, status, wasCollateralAccepted);
    }
...
void MasterNodesWidget::onInfoMNClicked(){
    ...
    QString label = index.data(Qt::DisplayRole).toString();
    QString address = index.sibling(index.row(), MNModel::IP_ADDRESS).data(Qt::DisplayRole).toString();
    QString status = index.sibling(index.row(), MNModel::STATUS).data(Qt::DisplayRole).toString();
...
void MasterNodesWidget::onCreateMNClicked(){
    if(verifyWalletUnlocked()) {
        if(walletModel->getBalance() <= (COIN * 25000)){
            inform(tr("Not enough balance to create a masternode, 25,000 FAW required."));
            return;
        }
        showHideOp(true);
