    ...
    ui(new Ui::SendCustomFeeDialog)
{
    ...
    ui->lineEditCustomFee->setPlaceholderText("0.000001 FAW");
    ...
    connect(ui->checkBoxAutoFee, SIGNAL(clicked()), this, SLOT(updateAutoFee()));
    ...
    QSettings settings;
    if (settings.value("autoFee", false).toBool())
        ui->checkBoxAutoFee->setCheckState(Qt::Checked);
}
...
void SendCustomFeeDialog::updateAutoFee() {
    QSettings settings;
    settings.setValue("autoFee", ui->checkBoxAutoFee->checkState() == Qt::Checked);
}
