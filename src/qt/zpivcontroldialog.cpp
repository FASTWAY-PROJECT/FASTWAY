    ...
    model(0)
{
    ...
    // Title
    ui->labelTitle->setText(tr("Select zFAW Denominations to Spend"));
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zFAW.";
            else if (!mint.isSeedCorrect)
                strReason = "The zFAW seed used to mint this zFAW is not the same as currently hold in the wallet";
            else
