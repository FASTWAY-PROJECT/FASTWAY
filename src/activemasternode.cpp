...
//
// Bootup the Masternode, look for a 25000 FAW input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if (status == ACTIVE_MASTERNODE_INITIAL) {
        ...
        if (pmn != NULL) {
            pmn->Check();
            if (pmn->IsEnabled(true) && pmn->protocolVersion >= masternodePayments.GetMinMasternodePaymentsProto()) EnableHotColdMasterNode(pmn->vin, pmn->addr);
        }
    }
