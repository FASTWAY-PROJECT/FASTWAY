...
bool Stake(const CBlockIndex* pindexPrev, CStakeInput* stakeInput, unsigned int nBits, unsigned int& nTimeTx, uint256& hashProofOfStake)
{
    ...
    // iterate the hashing
    bool fSuccess = false;
    const unsigned int nHashDrift = 25;
...
bool ContextualCheckZerocoinStake(int nPreviousBlockHeight, CStakeInput* stake)
{
    if (nPreviousBlockHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s : zFAW stake block is less than allowed start height", __func__);

    if (CZPivStake* zPIV = dynamic_cast<CZPivStake*>(stake)) {
        CBlockIndex* pindexFrom = zPIV->GetIndexFrom();
        if (!pindexFrom)
            return error("%s : failed to get index associated with zFAW stake checksum", __func__);

        int depth = (nPreviousBlockHeight + 1) - pindexFrom->nHeight;
        if (depth < Params().Zerocoin_RequiredStakeDepth())
            return error("%s : zFAW stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, nPreviousBlockHeight, pindexFrom->nHeight);
...
bool initStakeInput(const CBlock block, std::unique_ptr<CStakeInput>& stake, int nPreviousBlockHeight) {
    ...
    if (txin.IsZerocoinSpend()) {
        ...
        if (!ContextualCheckZerocoinStake(nPreviousBlockHeight, stake.get()))
            return error("%s : staked zFAW fails context checks", __func__);
    } else {
